...
class CandleDB:
    ...
    def write_signal_history(
        ...
        store = _require(self._signal_history)
        store.write_signal_history(
            scored_at,
            symbol,
            horizon_bars,
            mean_return,
    ...
    def write_signal_history_batch(self, rows: list[SignalHistoryRow]) -> None:
        store = _require(self._signal_history)
        store.write_signal_history_batch(rows)

    def resolve_signal_history(self, now_ms: int) -> int:
        store = _require(self._signal_history)
        return store.resolve_signal_history(now_ms)

    def compute_gap_spanned_at_horizon(self, symbol: str, scored_at: int, horizon_bars: int) -> int:
        store = _require(self._signal_history)
        return store.compute_gap_spanned_at_horizon(symbol, scored_at, horizon_bars)
    ...
    def get_signal_history(
        ...
        store = _require(self._signal_history)
        return store.get_signal_history(
            symbol=symbol,
            since_ms=since_ms,
    ...
    def write_correlations(self, computed_at: int, matrix: dict[str, dict[str, float]]) -> None:
        corr = _require(self._correlations)
        corr.write_correlations(computed_at, matrix)

    def read_latest_correlations(self) -> dict[str, dict[str, float]]:
        corr = _require(self._correlations)
        return corr.read_latest_correlations()
    ...
    def insert_macro_observations(
        ...
        macro = _require(self._macro)
        return macro.insert_macro_observations(series_id, observations)

    def get_macro_series(self, series_id: str, limit: int | None = None) -> list[tuple[int, float]]:
        macro = _require(self._macro)
        return macro.get_macro_series(series_id, limit=limit)

    def get_latest_macro_value(self, series_id: str) -> tuple[int, float] | None:
        macro = _require(self._macro)
        return macro.get_latest_macro_value(series_id)
