"""SQLite-backed ``signal_history`` persistence.

Extracted from ``CandleDB`` as a dedicated store, sharing its connection.
Records every TopK prediction and, once its horizon elapses, the realised
outcome — the system of record for signal-quality diagnostics and
recalibration (``scripts/signal_diagnostics.py``, ``docs/calibration_runbook.md``).
"""
...
# ALTER TABLE migrations for existing live DBs.
# Each tuple is (column_name, ALTER statement). Applied idempotently in init_schema.
#
# 2026-06-01 — sentiment-edge measurement harness adds five columns so we can
# decide whether the sentiment overlay carries actual edge.  See
# ``docs/sentiment_edge_measurement.md``.  Coverage defaults to 0 so pre-
# migration rows are honestly bucketed as "absent" by the analysis script.
...
class SignalHistoryRow(TypedDict):
    ...
    between ``rerank_runner`` and :meth:`SignalHistoryStore.write_signal_history_batch`).
    ...
    :meth:`SignalHistoryStore.write_signal_history_batch` for the exact layouts.
    """
...
class SignalHistoryRecord(TypedDict):
    """Read shape returned by :meth:`SignalHistoryStore.get_signal_history` — one
    dict per row, keyed by column name.  ``realized_*`` stay ``None`` until
    :meth:`SignalHistoryStore.resolve_signal_history` fills them after the
    horizon elapses."""
...
class SignalHistoryStore:
    """Persistent SQLite store for TopK ``signal_history`` rows.
    ...
    def init_schema(self) -> None:
        """Create the table/indexes and apply any pending ALTER migrations."""
